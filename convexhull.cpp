...
// non va bene il convexhull per il primo test perchè ci sono punti allineati sulla x
namespace Delaunay
{
               ...
               const Point& p3)
{
  ...
  if (dx1*dy2 > dy1*dx2)
    ...
    return (dx1*dx2+dy1*dy2 >= 0);  // faccio prodotto scalare per vedere se i tre punti sono allineati, se prodotto scalare maggiore di zero allora l'angolo è superiore, altrimenti è inferiore
}
    ...
    vector<Triangle> TriangoliConvexHull (vector<Point> punti,vector <Lati>& lati,int& t_id,int& l_id)
         {
              ...
              for(int i=0;i<size;i++)  // sto inizializzando la triangolazione sia per i lati che per i triangoli
              {
